class Prey {
  ...
  display(shouldShowSensors) {
    ...
    if (this.health > 0) {
      if (random(1, 3) < 1.8) {
        this.particles.push(new particle(this.position.x, this.position.y, random(0.1, 0.3), [0, 0, random(180, 255)], false, 1.4));
      }
  ...
  setHealthBar() {
    ...
    fill(255, 0, 0);
    rect(this.position.x + this.width / 2, this.position.y + this.height / 1.5, -((this.maxHealth - this.health) / this.width * 10), this.height / 4);
  }
